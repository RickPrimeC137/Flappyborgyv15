...
class GameScene extends Phaser.Scene {
  ...
  spawnPair(silentFirst){
    ...
    // Robot SwissBorg décoratif mais létal
    if (this.started && this.pairsSpawned > 0 && this.pairsSpawned % 15 === 0){
      ...
      if (fromBottom) {
        ...
        // Hitbox généreuse autour du robot
        const bw = bot.displayWidth;
        const bh = bot.displayHeight;
        bot.body.setSize(bw*0.8, bh*0.95, true);

        this.bots.add(bot);

        const h = bot.displayHeight;
        const yHidden = bottomImg.y + h * 0.6;
        const yShown  = bottomImg.y + h * 0.1;
