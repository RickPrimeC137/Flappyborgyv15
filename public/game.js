/* FlappyBorgy — 1024x1536 (pipes light only + Telegram leaderboard)
   Domaine jeu  : https://flappyborgyv15.onrender.com
...
class PreloadScene extends Phaser.Scene{
  ...
  preload(){
    ...
    const barBg = this.add.rectangle(W/2, H*0.55, W*0.52, 12, 0x000000, 0.15).setOrigin(0.5);
    const barFg = this.add.rectangle(W*0.24, H*0.55, 2, 12, 0x17a689).setOrigin(0,0.5);
    const pct   = this.add.text(W/2, H*0.55+26, "0%", { fontFamily:"monospace", fontSize:18, color:"#044" }).setOrigin(0.5);
    this.load.on("progress", p => { barFg.width = (W*0.52)*p; pct.setText(Math.round(p*100)+"%"); });
