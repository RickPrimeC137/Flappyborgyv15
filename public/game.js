...
const PLAYFIELD_BOT_PCT = 0.9;
const PIPE_RIM_MAX_PCT = 0.8;
...
function ensureBgm(scene, opts = {}) {
  ...
  if (!gm._bgm || gm._bgm.destroyed === true) {
    gm._bgm = scene.sound.add(wantedKey, {
      loop: true,
      volume: 0.35
    });
  ...
  const start = () => {
    if (!gm._bgm.isPlaying) gm._bgm.play();
  };
...
// distance horizontale minimale entre deux paires de tuyaux (augmentée)
...
async function postScore(score, isHard = false) {
  ...
  try {
    ...
    console.warn("score post error", e);
  }
}
    ...
    if (raw) {
      ...
      if (data && Array.isArray(data.skins) && data.skins.length) {
        ...
        if (
          !data.selectedId ||
          !data.skins.some(
            s => s.id === data.selectedId && s.owned
          )
        ) {
          const fallback =
            data.skins.find(s => s.owned) || data.skins[0];
          if (fallback) {
            data.selectedId = fallback.id;
          }
        }
        data.skins.forEach(s => {
          s.selected = s.id === data.selectedId;
        });
...
class PreloadScene extends Phaser.Scene {
  ...
  preload() {
    ...
    const bgBar = this.add
      .rectangle(W / 2, H * 0.8, W * 0.52, 12, 0x000000, 0.25)
      .setOrigin(0.5);
    const fgBar = this.add
      .rectangle(W * 0.24, H * 0.8, 2, 12, 0x17a689)
      .setOrigin(0, 0.5);
    const pct = this.add
      .text(W / 2, H * 0.8 + 26, "0%", {
...
class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (typeof this.game._muted === "undefined")
      ...
      this.game._bgm?.setMute(this.game._muted);
    }
    muteBtn.on("pointerdown", () => {
      const s = this.game._bgm;
      const m = this.game._muted === true;
      s?.setMute(!m);
      this.game._muted = !m;
      muteBtn.setText(this.game._muted ? "🔇" : "🔊");
    });
    ...
    this.add
      .text(W / 2, H * 0.19, `Borgy Coins : ${totalCoins}`, {
        fontFamily: "monospace",
        fontSize: 30,
        color: "#0b4a44"
      })
    ...
    this.makeBtn(W / 2, H * 0.46, "Quêtes 🔥", () =>
      this.showQuests()
    );
    this.makeBtn(
      W / 2,
    ...
    this.makeBtn(
      ...
      () => {
        const url =
          "https://lewk.com/vote/BorGY4ub2Fz4RLboGxnuxWdZts7EKhUTB624AFmfCgX";
    ...
    this.makeBtn(W / 2, H * 0.7, "Buy Borgy", () => {
    ...
    if (typeof this.game._hardMode === "undefined") {
      try {
        this.game._hardMode = JSON.parse(
          localStorage.getItem("flappy_borgy_hard") || "false"
        );
      } catch {
        this.game._hardMode = false;
      }
    }
    const hardBtn = this.makeBtn(
      ...
      () => {
        ...
        localStorage.setItem(
          "flappy_borgy_hard",
          JSON.stringify(this.game._hardMode)
        );
        hardBtn.setText(
          this.game._hardMode
            ? "Mode Hard : ON"
        ...
        hardBtn.setBackgroundColor(
          this.game._hardMode ? "#b91c1c" : "#12a38a"
        );
      }
    );
    hardBtn.setBackgroundColor(
    ...
    this.add
      .text(
        W / 2,
        H * 0.92,
        "Tap/Espace pour sauter — évitez les tuyaux",
        {
          fontFamily: "monospace",
          fontSize: 22,
          color: "#0b4a44",
  ...
  showLeaderboard(list, isHard = false) {
    ...
    list.slice(0, 10).forEach((row, i) => {
      ...
      this.add
        .text(colX, y, String(i + 1).padStart(2, "0") + ".", {
          fontFamily: "monospace",
          fontSize: 36,
          color: "#bff"
        })
    ...
    const close = this.add
      ...
      .setInteractive({ useHandCursor: true });
    const destroyAll = () =>
      [panel, title, close, ...this.children.list.filter(
        o => o.depth >= depth && !o.input
      )].forEach(o => o?.destroy());
  ...
  showQuests() {
    ...
    const panel = this.add
      .rectangle(W / 2, H * 0.5, W * 0.82, H * 0.58, 0x062b35, 0.94)
      .setDepth(depth);
    const title = this.add
      .text(W / 2, H * 0.26, "Quêtes du jour", {
        fontFamily: "Georgia,serif",
        fontSize: 60,
        color: "#ffffff"
      })
    ...
    data.quests.forEach((q, i) => {
      ...
      this.add
        .text(W * 0.14, y, q.title, {
          fontFamily: "monospace",
          fontSize: 30,
          color: q.done ? "#b3ffcf" : "#fff"
        })
        .setOrigin(0, 0.5)
        .setDepth(depth + 1);
      const barW = W * 0.38,
        barX = W * 0.54;
      this.add
        .rectangle(barX, y, barW, 12, 0xffffff, 0.15)
        .setOrigin(0, 0.5)
        .setDepth(depth + 1);
      this.add
        .rectangle(
      ...
      this.add
        .text(W * 0.14, y + 28, `Récompense: ${rewardTxt}`, {
          fontFamily: "monospace",
          fontSize: 18,
          color: "#c3ede5"
        })
  ...
  showShop() {
    ...
    // Panneau principal
    const panel = this.add
      .rectangle(W / 2, H * 0.5, W * 0.8, H * 0.55, 0x05252f, 0.96)
      .setDepth(depth + 1);
    elements.push(panel);

    const title = this.add
      .text(W / 2, H * 0.26, "Borgy Coins Shop", {
        fontFamily: "Georgia,serif",
        fontSize: 54,
        color: "#ffffff"
      })
    ...
    const infoText = this.add
      .text(W * 0.5, H * 0.38, "Choisis ton skin Borgy :", {
        fontFamily: "monospace",
        fontSize: 22,
        color: "#9be7ff",
        align: "center"
    ...
    if (skinState && Array.isArray(skinState.skins)) {
      skinState.skins.forEach((skin, i) => {
        ...
        if (skin.id === "borgy_gold") {
          ...
          noteLabel = "Bonus Swissborg x3";
        } else if (skin.id === "borgy_diamant") {
          noteLabel = "1 vie supplémentaire";
        }

        if (noteLabel) {
          const noteTxt = this.add
            .text(W * 0.16, y + 22, noteLabel, {
              fontFamily: "monospace",
              fontSize: 18,
              color: "#e5f2ff"
            })
  ...
  showWelcomePopup() {
    ...
    const displayWidth = this.scale.displaySize
      ? this.scale.displaySize.width
      : window.innerWidth || W;
    const displayHeight = this.scale.displaySize
      ? this.scale.displaySize.height
      : window.innerHeight || H;
    const isMobileLike =
      displayWidth < 800 || displayHeight > displayWidth;
    ...
    // panneau principal
    const panel = this.add
      .rectangle(W / 2, H / 2, panelWidth, panelHeight, 0x08313a, 0.96)
      .setDepth(depthPanel);
    elements.push(panel);
    ...
    const textX = W * 0.2; // colonne texte

    let y = H * 0.32;

    // 1) Contrôles (pas d'icône)
    const txtControls = this.add
      .text(
        textX,
        y,
    ...
    const bonusIcon = this.add
      .image(iconX, y + 16, "bonus_sb")
      .setOrigin(0.5, 0)
      .setDepth(depthPanel + 2);
    ...
    // Texte final
    const txtEnd = this.add
      .text(
        W * 0.16,
        isMobileLike ? H * 0.68 : H * 0.7,
    ...
    const okBtn = this.add
      .text(W / 2, H * 0.84, "OK, c'est parti !", {
        fontFamily: "monospace",
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    const bg = this.add
      .image(W / 2, H / 2, hasKey ? keyWanted : BG_KEY)
      .setDepth(-10);
    bg
      .setScale(Math.max(W / bg.width, H / bg.height))
      .setScrollFactor(0);
    ...
    // ===== Nuages haut / bas =====
    {
      ...
      const bottomCloudVisualHeight =
        bottomCloudHeight * 1.8; // un peu plus pour bien remplir le bas

      // --- Nuage du haut : déborde au-dessus de l'écran ---
      const topCloudY = topCloudHeight - topCloudVisualHeight / 2;
      this.topCloud = this.add
        .image(W / 2, topCloudY, "cloud_top")
        .setDepth(5);

      const topScaleX =
        (W * CLOUD_EXTRA_SCALE_X) / this.topCloud.width;
      const topScaleY =
        topCloudVisualHeight / this.topCloud.height;
      this.topCloud.setScale(topScaleX, topScaleY);

      // --- Nuage du bas : déborde sous l'écran ---
      const bottomCloudY =
        H - bottomCloudHeight + bottomCloudVisualHeight / 2;
      this.bottomCloud = this.add
        .image(W / 2, bottomCloudY, "cloud_bottom")
        .setDepth(5);

      const bottomScaleX =
        (W * CLOUD_EXTRA_SCALE_X) / this.bottomCloud.width;
      const bottomScaleY =
        bottomCloudVisualHeight / this.bottomCloud.height;
      ...
      this.topCloud.body.setSize(
        W * CLOUD_EXTRA_SCALE_X,
        topCloudHeight,
        true
      );
      this.topCloud.body.setOffset(
        (-W * (CLOUD_EXTRA_SCALE_X - 1)) / 2,
        0
      );

      this.bottomCloud.body.setSize(
        W * CLOUD_EXTRA_SCALE_X,
        bottomCloudHeight,
        true
      );
    ...
    this.bots = this.physics.add.group(); // robots SwissBorg décoratifs mais mortels
    ...
    this.borgyCoinText = this.add
      .text(W - 24, 18, `🪙 ${this.borgyCoinCount}`, {
        fontFamily: "monospace",
        fontSize: 36,
        color: "#fff",
        stroke: "#0a3a38",
    ...
    const pw = this.player.displayWidth,
      ph = this.player.displayHeight;
    this.player.body
      .setSize(pw * 0.5, ph * 0.5, true)
      .setOffset(pw * 0.2, ph * 0.22);
    ...
    if (ENABLE_KILL_BANDS) {
      ...
      this.killTop = this.add
        .rectangle(W / 2, topBand / 2, W, topBand, 0, 0)
        .setDepth(0);
      this.physics.add.existing(this.killTop, true);
      this.killBottom = this.add
        .rectangle(W / 2, (H + botBand) / 2, W, H - botBand, 0, 0)
        .setDepth(0);
    ...
    // Robot Swissborg : hitbox custom et mortel
    this.physics.add.overlap(
      this.player,
      this.bots,
      () => this.gameOver(),
    ...
    this.time.addEvent({
      ...
      loop: true,
      callback: () => {
        this.curSpeed = Math.max(
          DIFF.minSpeed,
          this.curSpeed + DIFF.speedDelta
        );
        this.curDelay = Math.max(
          DIFF.minDelay,
          this.curDelay + DIFF.delayDelta
        );
        if (this.started) {
          const effDelay = this._getSpawnDelay();
          this.nextSpawnAt = Math.max(
            this.time.now + effDelay,
            this.nextSpawnAt
          );
  ...
  _forceVelocities() {
    this.pipes.children.iterate(p => {
      if (p?.body) p.body.setVelocityX(this.curSpeed);
    });
    this.sensors.children.iterate(s => {
      if (s?.body) s.body.setVelocityX(this.curSpeed);
    });
    this.bonuses.children.iterate(b => {
      if (b?.body) b.body.setVelocityX(this.curSpeed);
    });
    this.borgyCoins.children.iterate(c => {
      if (c?.body) c.body.setVelocityX(this.curSpeed);
    });
    this.bots.children.iterate(b => {
      if (b?.body) b.body.setVelocityX(this.curSpeed);
    });
  }
  ...
  _flashStormClouds() {
    ...
    const baseTop =
      this._stormBaseTopTint ?? this.topCloud.tintTopLeft;
    const baseBottom =
      this._stormBaseBottomTint ?? this.bottomCloud.tintTopLeft;
  ...
  update() {
    ...
    this.player.setAngle(
      vy < -40 ? -16 : vy > 140 ? 20 : 0
    );

    if (this.started && this.time.now >= this.nextSpawnAt) {
      if (
        this.lastSpawnMs < 0 ||
        this.time.now - this.lastSpawnMs >= DIFF.cooldownMs
      ) {
        this.spawnPair(false);
        this.lastSpawnMs = this.time.now;
      }
    ...
    this.pipes.children.iterate(p => {
      if (
        p &&
        p.active &&
        p.x + p.displayWidth * 0.5 < -KILL_MARGIN
      )
        p.destroy();
    });
    this.sensors.children.iterate(s => {
      if (s && s.active && s.x < -KILL_MARGIN) s.destroy();
    });
    this.bonuses.children.iterate(b => {
      if (b && b.active && b.x < -KILL_MARGIN) b.destroy();
    });
    this.borgyCoins.children.iterate(c => {
    ...
    this.pipePairs = this.pipePairs.filter(
      pair =>
        pair &&
        pair.top &&
        pair.bottom &&
    ...
    if (
      this.multiplierActive &&
      this.bonusFollower &&
      this.player.active
    ) {
      this.bonusFollower.x =
    ...
    if (this.DEBUG && this.debugTxt) {
      this.debugTxt.setText(
        `speed:${this.curSpeed}  delay:${this.curDelay}  next:${Math.max(
          0,
          Math.ceil(this.nextSpawnAt - this.time.now)
        )}ms`
      );
    }
  }
  ...
  _resizePipeToRim(img, isTop, rimY, scaleX) {
    ...
    img.body.setOffset(
      (displayW - bodyW) / 2,
      isTop ? img.displayHeight - img.body.height : 0
    );
  }
  ...
  spawnPair(silentFirst) {
    ...
    let maxY = Math.min(
      BOT_BAND - Math.floor(GAP / 2),
      RIM_LIMIT - Math.floor(GAP / 2) + PAD
    );
    if (maxY < minY) {
    ...
    const topImg = this.physics.add
      .image(x, 0, "pipe_top")
      .setDepth(6)
      .setOrigin(0.5, 1);
    const bottomImg = this.physics.add
      .image(x, 0, "pipe_bottom")
      .setDepth(6)
      .setOrigin(0.5, 0);
    ...
    let yTopRim0 = Math.round(
      gapY - GAP / 2 + (PAD - JOINT_OVERLAP)
    );
    let yBottomRim0 = Math.round(
      gapY + GAP / 2 - (PAD - JOINT_OVERLAP)
    );
    ...
    if (
      ENABLE_BONUS &&
      this.started &&
      this.pairsSpawned % BONUS_EVERY === 0
    ) {
      const bonusX = x;
      const bonusY = gapCenterY;
      const bonus = this.physics.add
        .image(bonusX, bonusY, "bonus_sb")
        .setDepth(7)
        .setScale(0.55)
        .setImmovable(true);
      ...
      bonus.body.setSize(
        bonus.displayWidth * 3.0,
        bonus.displayHeight * 3.0,
        true
      );
    ...
    if (
      ...
      if (fromBottom) {
        ...
        // hitbox réduite pour être plus fair-play
        const bw = bot.displayWidth * 0.55;
        const bh = bot.displayHeight * 0.8;
        bot.body.setSize(bw, bh, true);

        this.bots.add(bot);

        const h = bot.displayHeight;
        const yHidden = bottomImg.y + h * 0.6; // robot caché dans le tuyau
        const yShown = bottomImg.y + h * 0.1; // moitié du corps qui dépasse
      ...
      // --- tuyau du haut (sprite inversé verticalement) ---
        ...
        const h = bot.displayHeight;
        const yHidden = topImg.y - h * 0.6; // robot caché dans le tuyau
        const yShown = topImg.y - h * 0.1; // moitié supérieure visible
      ...
      if (amp > 0) {
        ...
        const tween = this.tweens.add({
          ...
          onUpdate: () => {
            ...
            const topClamped = Phaser.Math.Clamp(
              newTop,
              TOP_BAND + 10,
              RIM_LIMIT - 10
            );
            const bottomClamped = Phaser.Math.Clamp(
              newBottom,
              TOP_BAND + 10,
              BOT_BAND - 10
            );

            this._resizePipeToRim(
              topImg,
              true,
              topClamped,
              scaleXt
  ...
  handleBorgyCoinOverlap(_player, coin) {
  ...
  addScore(n) {
    ...
    if (!isHard) {
      ...
      if (this.score >= 200) {
        ...
        const newSpeedStep =
          Math.floor(extraScore / 50) + 1; // 200–249 => 1, 250–299 => 2, etc.
  ...
  _findNearestPipePairAhead() {
    if (!this.pipePairs || !this.pipePairs.length || !this.player)
      return null;
  ...
  _finalGameOver() {
    ...
    const W = this.scale.width,
      H = this.scale.height;
    this.add
      .rectangle(W / 2, H / 2, W * 0.8, 360, 0x12323a, 0.92)
      .setDepth(100);
    this.add
      ...
      .setDepth(101);
    this.add
      .text(W / 2, H / 2 - 28, `Score : ${this.score}`, {
        fontFamily: "monospace",
        fontSize: 48,
        color: "#cffff1"
      })
    ...
    const menuBtn = this.add
      .text(W / 2, H / 2 + 140, "Menu principal", {
        fontFamily: "monospace",
        fontSize: 40,
        color: "#fff",
    ...
    postScore(this.score, isHard).then(() =>
      fetchLeaderboard(10, isHard).then(list => {
        if (list?.length) this.showLeaderboard(list, isHard);
      })
    );
  }
